...
public class SharkBehaviour : MonoBehaviour
{
    ...
    void Start() {
        ...
        if (name.StartsWith("meteor")) speed = 2f;
    ...
    void Update() {
        if (Vector3.Distance(transform.position, target.position) < sightRange) {
            Vector3 asCloseAsItCanGet = new Vector3(target.position.x, target.position.y, target.position.z);
